  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.matchHeader}>
        <Text style={[styles.leagueInfo, { color: 'rgba(255,255,255,0.8)' }]}>
        ...
        <View style={styles.teamsRow}>
          ...
          <View style={styles.teamBox}>
            {badges[event.homeTeam] ? (
              ...
              <Text style={[styles.teamCode, { color: '#fff' }]}>
                {event.homeTeam.substring(0, 3).toUpperCase()}
              </Text>
            )}
            <Text style={[styles.teamFullName, { color: 'rgba(255,255,255,0.9)' }]}>{event.homeTeam}</Text>
            <TouchableOpacity
          ...
          <View style={styles.scoreCenter}>
            <Text style={[styles.bigScore, { color: '#fff' }]}>
              {event.homeScore} - {event.awayScore}
            </Text>
            <Text style={[styles.finalText, { color: 'rgba(255,255,255,0.8)' }]}>Finalizado</Text>
          </View>
          ...
          <View style={styles.teamBox}>
            ...
            <Text style={[styles.teamFullName, { color: 'rgba(255,255,255,0.9)' }]}>{event.awayTeam}</Text>
            <TouchableOpacity
              style={styles.starBtn}
        ...
        <View style={[styles.section, { backgroundColor: colors.surface }]}>
          ...
          {stats.map((stat, index) => (
            <View key={index} style={[styles.statRow, { borderBottomColor: colors.border }]}>
              <Text style={[styles.statValue, { color: '#CC0000' }]}>
                {stat.strHomeTeamStat ?? "-"}
              </Text>
              <Text style={[styles.statLabel, { color: colors.textSecondary }]}>{stat.strStat}</Text>
              <Text style={[styles.statValue, { color: '#CC0000' }]}>
      ...
      <View style={[styles.section, { backgroundColor: colors.surface }]}>
        ...
        {detail?.strVenue ? (
          <Text style={[styles.infoText, { color: colors.text }]}>🏟️ {detail.strVenue}</Text>
        ) : null}
        {detail?.strReferee ? (
          <Text style={[styles.infoText, { color: colors.text }]}>👤 Árbitro: {detail.strReferee}</Text>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  teamCode: { fontSize: 28, fontWeight: "bold" },
  teamFullName: {
    fontSize: 13,
    marginTop: 4,
    textAlign: "center",
  ...
  bigScore: { fontSize: 42, fontWeight: "bold" },
  finalText: { fontSize: 12, marginTop: 4 },
  section: {
    margin: 15,
    borderRadius: 12,
    padding: 15,
  ...
  infoText: { fontSize: 14, paddingVertical: 6 },
});
